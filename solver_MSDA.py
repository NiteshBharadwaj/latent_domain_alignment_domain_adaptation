...
from model.build_gen_digits import Generator, Classifier, DomainPredictor
...
class Solver(object):
    def __init__(self, args, batch_size=64,
        ...
        if args.dl_type == 'original':
            ...
            self.datasets, self.dataset_test, self.dataset_valid, self.classwise_dataset, self.is_multi, self.usps_only = dataset_combined(
                target, self.batch_size, args.num_domain, args.office_directory, args.seed,
        ...
        self.G = Generator(cd=class_disc, usps_only=self.usps_only)
        self.C1 = Classifier(cd=class_disc, usps_only=self.usps_only)
        self.C2 = Classifier(cd=class_disc, usps_only=self.usps_only)
        self.DP = DomainPredictor(num_domains, cd=class_disc, usps_only=self.usps_only)
    ...
    def loss_soft_all_domain(self, img_s, img_t, label_s, epoch, img_s_cl, force_attach=False,
        ...
        if self.to_detach and not force_attach:
            loss_msda_nc2, loss_msda_nc1 = msda.msda_regulizer_soft(feat_da_s, feat_da_t, 5, domain_prob.detach(),
                                                                    single_domain_mode=single_domain_mode)
        else:
            loss_msda_nc2, loss_msda_nc1 = msda.msda_regulizer_soft(feat_da_s, feat_da_t, 5, domain_prob,
                                                                    single_domain_mode=single_domain_mode)
