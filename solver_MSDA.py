...
class Solver(object):
    ...
    def train(self, epoch, record_file=None):
        ...
        for batch_idx, data in enumerate(self.datasets):
            ...
            loss_msda = 0.5 * msda.msda_regulizer(feat_s1, feat_s2, feat_s3, feat_s4, feat_t, 5) / (
    ...
    def test(self, epoch, is_train_perf, record_file=None, save_model=False):
        self.G.eval()
        self.C1.eval()
        test_loss = 0
        correct1 = 0
    ...
    def train_MSDA_soft(self, epoch, record_file=None):
        ...
        for batch_idx, data in enumerate(self.datasets):
            ...
            #         epoch, batch_idx, 100,
            #         100. * batch_idx / 70000, loss_s_c1.data.item(), loss_s_c2.data.item(), 0, 0, 0))
            #     if record_file:
            #         record = open(record_file, 'a')
            #         record.write('%s %s %s %s %s %s\n' % (0, loss_s_c1.data.item(), loss_s_c2.data.item(), 0, 0, 0))
            #         record.close()
        return batch_idx_g
