...
def train_MSDA_soft(solver, epoch, classifier_disc=True, record_file=None):
    ...
    for batch_idx, data in enumerate(solver.datasets):
        ...
        loss_s_c1, loss_s_c2, loss_msda, entropy_loss, aux_loss = solver.loss_soft_all_domain(img_s, img_t, label_s)

        loss = loss_s_c1 + loss_msda + loss_s_c2 + entropy_loss + aux_loss
        ...
        if classifier_disc:
            ...
            loss_s = loss_s_c1 + loss_msda + loss_s_c2 + entropy_loss + aux_loss
        ...
        if batch_idx % solver.interval == 0:
            print \
                ('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss1: {:.6f}\t Loss2: {:.6f}\t Loss_mmd: {:.6f}\t Loss_entropy: {:.6f}\t aux_loss: {:.6f}'.format(
                epoch, batch_idx, 100, 100. * batch_idx / 70000, loss_s_c1.data.item(), loss_s_c2.data.item(), loss_msda.data.item(), entropy_loss.data.item(), aux_loss.data.item()))
            if record_file:
                record = open(record_file, 'a')
                record.write('%s %s %s %s %s %s\n' %
                (0, loss_s_c1.data.item(), loss_s_c2.data.item(), loss_msda.data.item(), entropy_loss.data.item(), aux_loss.data.item()))
                record.close()
    return batch_idx_g