...
def moment_soft(want_class_tear_apart, output_s, output_t, class_prob_s, class_prob_t, domain_prob_s, label_s, moment, domain_prob_t):
    margin = 30
    ...
    domain_prob_t = domain_prob_t.reshape(domain_prob_t.shape[0], 1, domain_prob_t.shape[1],
                                          domain_prob_t.shape[2])  # SHAPE -> N x 1 x c x d
    output_t_times_cp = output_t_times_cp.reshape(output_t_times_cp.shape[0], output_t_times_cp.shape[1],
                                                  output_t_times_cp.shape[2], 1)  # SHAPE -> N x e x c x 1
    output_prob_t = output_t_times_cp * domain_prob_t  # SHAPE -> N x e x c x d
    class_prob_sum_t = class_prob_t.sum(0) + 1e-6
    class_prob_sum_t = class_prob_sum_t.reshape(1, -1) #SHAPE -> 1 x c
    domain_prob_sum_t = domain_prob_t.sum(0) + 1e-6
    domain_prob_sum_t = domain_prob_sum_t.reshape(1, c, d)
    output_prob_t = output_prob_t.sum(0)/domain_prob_sum_t # SHAPE -> e x c x d
            ...
            for dd2 in range(dd, output_prob_s.shape[2]):
                ...
                if dd2!=dd:
                    intra_domain_loss += domain_prob_sum_s[0,cc,dd]*domain_prob_sum_s[0,cc,dd2]*euclidean(
                        output_prob_s[:, cc, dd], output_prob_s[:, cc, dd2])/(output_s.shape[0]**2)
...
def k_moment_soft(want_class_tear_apart, output_s, output_t, k, class_prob_s, class_prob_t, domain_prob_s, label_s,domain_prob_t):
    output_s_k = output_s**k
    output_t_k = output_t**k
    return moment_soft(want_class_tear_apart, output_s_k, output_t_k, class_prob_s, class_prob_t, domain_prob_s, label_s, k, domain_prob_t)

def class_da_regulizer_soft(want_class_tear_apart, output_s, output_t, belta_moment, class_prob_s, class_prob_t, domain_prob_s, label_s, domain_prob_t):
    # print('s1:{}, s2:{}, s3:{}, s4:{}'.format(output_s1.shape, output_s2.shape, output_s3.shape, output_t.shape))        
    intra_domain_loss = torch.zeros(1).cuda()
    inter_domain_loss = torch.zeros(1).cuda()
    class_tear_apart_loss = torch.zeros(1).cuda()
    for i in range(0,belta_moment):
        klosses = k_moment_soft(want_class_tear_apart, output_s, output_t, i + 1, class_prob_s, class_prob_t, domain_prob_s, label_s, domain_prob_t)
