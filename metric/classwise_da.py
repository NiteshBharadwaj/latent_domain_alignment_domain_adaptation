...
def moment_soft(output_s, output_t, class_prob_s, class_prob_t):
    ...
    output_prob_t = torch.matmul(output_t, class_prob_t)
    output_prob_sum_t = class_prob_t.sum(0)
    output_prob_t = output_prob_t.sum(0)/output_prob_sum_t.reshape(1, class_prob_t.shape[2])

    classwise_da_loss = 0
    for i in range(output_prob_s.shape[1]):
        classwise_da_loss += output_prob_sum_s[0,i] * output_prob_sum_t[0,i] * euclidean(output_prob_s[:,i], output_prob_t[:,i]) / (output_s.shape[0]**2)
