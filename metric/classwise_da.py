...
def moment_soft(output_s, output_t, class_prob_s, class_prob_t, domain_prob_s):
    ...
    domain_prob_s = domain_prob_s.reshape(domain_prob_s.shape[0], 1, 1, domain_prob_s.shape[1]) #SHAPE -> N x 1 x 1 x d
    output_s_times_cp = torch.matmul(output_s, class_prob_s) #SHAPE -> N x e x c
    output_s_times_cp = output_s_times_cp.reshape(output_s_times_cp[0], output_s_times_cp[1], output_s_times_cp[2], 1) #SHAPE -> N x e x c x 1
    output_prob_s = torch.matmul(output_s_times_cp, domain_prob_s) #SHAPE -> N x e x c x d
    ...
    domain_prob_sum_s = domain_prob_sum_s.reshape(1, 1, -1) # SHAPE -> 1 x 1 x d

    output_prob_s = output_s.shape[0] * ((output_prob_s.sum(0)/class_prob_sum_s)/domain_prob_sum_s) #SHAPE -> e x c x d
    ...
    for cc in range(output_prob_s.shape[1]):
        for dd in range(output_prob_s.shape[2]):
            inter_domain_loss += class_prob_sum_s[0,cc,0]*domain_prob_sum_s[0,0,dd]*class_prob_sum_t[0,cc]*euclidean(output_prob_s[:, cc, dd], output_prob_t[:, cc])/(output_s.shape[0]**3)
            for dd2 in range(dd+1, output_prob_s.shape[2]):
                intra_domain_loss += class_prob_sum_s[0,cc,0]*domain_prob_sum_s[0,0,dd]*class_prob_sum_s[0,cc,0]*domain_prob_sum_s[0,0,dd2]*euclidean(output_prob_s[:, cc, dd], output_prob_s[:, cc, dd2])/(output_s.shape[0]**4)
    return intra_domain_loss, inter_domain_loss


def k_moment_soft(output_s, output_t, k, class_prob_s, class_prob_t, domain_prob_s):
    output_s_k = output_s**k
    output_t_k = output_t**k
    return moment_soft(output_s_k, output_t_k, class_prob_s, class_prob_t, domain_prob_s)

def class_da_regulizer_soft(output_s, output_t, belta_moment, class_prob_s, class_prob_t, domain_prob_s):
    # print('s1:{}, s2:{}, s3:{}, s4:{}'.format(output_s1.shape, output_s2.shape, output_s3.shape, output_t.shape))        
    intra_domain_loss = 0
    inter_domain_loss = 0
    for i in range(0,belta_moment):
        klosses = k_moment_soft(output_s, output_t, i + 1, class_prob_s, class_prob_t, domain_prob_s)
