...
parser.add_argument('--weight_reinit', type=int, default=0, metavar='N',
                    help='want reinit weights in resnet DP??')
...
parser.add_argument('--grad_clip', type=int, default=0, metavar='LR',
                    help='want to do gradient clipping?')
parser.add_argument('--fine_tune_lr_ratio', type=float, default=0.1, metavar='LR',
                    help='fine_tune_lr_ratio (default: 0)')
parser.add_argument('--aux_loss', type=int, default=0, metavar='LR',
                    help='aux_loss = 2 => Intensity else 1 pose (default: 0)')
...
parser.add_argument('--is_fine_tune', type=int, default=0, metavar='N',
                    help='want fine tune ResNet')
parser.add_argument('--extra_fc', type=int, default=1, metavar='N',
                    help='want more fc in generator ?')
            ...
            if not args.one_step:
                ...
                if args.dl_type == 'soft_cluster':
                    ...
                    if args.data == 'digits':
                        ...
                        num = train_MSDA_soft_cars(solver, t, classifier_disc, record_file=record_train, summary_writer=summary_writer, epoch_start_idx=count)
                    else:
                        print("WTF Noob")
                    ...
                    torch.cuda.empty_cache()
                    num = train_MSDA_soft_cars(solver, t, classifier_disc, record_file=record_train, single_domain_mode=True)
