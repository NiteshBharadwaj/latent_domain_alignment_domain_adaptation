    ...
    def __getitem__(self, index):
        ...
        for i in range(self.n_datas):
            if index < self.len_end[i]:
                ...
                break
        assert data_idx != -1000
        data, labels, dom_labels = self.data[data_idx], self.labels[data_idx], self.domain_labels[data_idx]
        img, target, domain_labels = data[index % len(data)], labels[index % len(labels)], dom_labels[index % len(dom_labels)]
        # doing this so that it is consistent with all other datasets
        # to return a PIL Image
        # print(img.shape)
        if img.shape[0] != 1:
        ...
        if self.transform is not None:
            img = self.transform(img)
            # return img, target
        return img, target, domain_labels
