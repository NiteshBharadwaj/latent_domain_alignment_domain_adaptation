...
class DatasetReal(data.Dataset):
    ...
    def __init__(self, data, label, clusters, transform=None, target_transform=None):
        self.transform = transform
        self.target_transform = target_transform
        self.data = data
        self.labels = label
    ...
    def __getitem__(self, index):
        ...
        num = randrange(4)
        cluster = self.clusters.get(img_path, num)
        img = Image.open(img_path)
        img = np.array(img)
        avg_pixel_val = np.mean(img)
        intensity_val = avg_pixel_val//43
        ...
        return_tar = [float(i) for i in target]
        return_tar[2] = intensity_val
        return img, torch.Tensor(return_tar), cluster
