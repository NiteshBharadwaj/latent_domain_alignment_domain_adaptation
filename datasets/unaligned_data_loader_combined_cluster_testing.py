...
from datasets_ import Dataset, Dataset2, Dataset3
...
class CombinedDataDomain(Dataset):
    def __init__(self, data_loader_s, data_loader_t, max_dataset_size):
        super(Dataset, self).__init__()
        self.data_loader_s = data_loader_s
        self.data_loader_t = data_loader_t
    ...
    def __getitem__(self, index):
        S, S_paths,S_dom_labels , t, t_paths = None, None, None, None, None
        try:
            S, S_paths,S_dom_labels = next(self.data_loader_s_iter)
        except StopIteration:
            if S is None or S_paths is None or S_dom_labels is None:
                self.stop_s = True
                self.data_loader_s_iter = iter(self.data_loader_s)
                S, S_paths,S_dom_labels  = next(self.data_loader_s_iter)
        try:
        ...
        if (self.stop_s and self.stop_t) or self.iter > self.max_dataset_size:
            ...
            self.iter += 1
            return {'S': S, 'S_label': S_paths, 'S_domain_label': S_dom_labels,
                    'T': t, 'T_label': t_paths}
...
class UnalignedDataLoaderDomain():
    def initialize(self, source, target, batch_size1, batch_size2, scale=32):
        transform = transforms.Compose([
            transforms.Scale(scale),
            transforms.ToTensor(),
        ...
        for i in range(len(source)):
            imgs.append(source[i]['imgs'])
            labels.append(source[i]['labels'])
            domain_labels.append(source[i]['domain_labels'])

        dataset_source = Dataset3(imgs, labels, domain_labels , transform=transform)
        data_loader_s = torch.utils.data.DataLoader(dataset_source, batch_size=batch_size1, shuffle=True, num_workers=4)
        ...
        self.dataset_t = dataset_target
        self.paired_data = CombinedDataDomain(data_loader_s, data_loader_t, float("inf"))

        self.num_samples = min(max(len(dataset_source), len(self.dataset_t)), float("inf")) * 2

    def name(self):
        return 'UnalignedDataLoaderDomain'
