...
def train_source_only(solver, epoch, record_file=None, max_it=100000, prev_count=0):
    solver.G.train()
    solver.C1.train()
    solver.C2.train()
    solver.DP.train()
    ...
    for batch_idx, data in enumerate(solver.datasets):
        ...
        if batch_idx % 10 == 0:
            print \
             ...
             epoch, batch_idx, 100, 100. * batch_idx / 70000, loss_s_c1.data.item(), loss_s_c1.data.item(), 0, 0, 0))
#             if record_file:
